...
test("keeps the permanent key on the server and configures live Recall voice", async () => {
  ...
  assert.match(page, /arrivalVoiceScript/);
  assert.match(page, /words: "Hi\.", pauseAfterMs: 1500/);
  assert.match(page, /words: "I’m Ara\.", pauseAfterMs: 1250/);
  ...
  assert.match(styles, /araArrivalBreath[\s\S]*?1 both/);
